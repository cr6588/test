...
var siping = {
    ...
    ajax : function(opt) {
        var index = layer.load(0, 2);
        function fn() {}
        var url = opt.url || "", async = opt.async !== false, method = opt.type || 'GET', datas = opt.data || null, success = opt.success || fn;
        $.ajax({type : method,url : url,async : async,data : datas,dataType : "json",success : function(data){
                setTimeout(function(){layer.close(index);success(data);},500);}});
    },
    get : function(url, data, success) {
        ...
        $.get(url, data, function(data){setTimeout(function(){layer.close(index);success(data);},500);});
    },
    post : function(url, data, success) {
        ...
        $.post(url, data, function(data){setTimeout(function(){layer.close(index);success(data);},500);});
    }
};
...
//			var i_index = parent.layer.getFrameIndex(window.name);
//			parent.layer.iframeAuto(i_index);
		});
	}
	...
	/*左侧菜单*/	
	/*$(".menu_dropdown dl dt").click(function(){$.Huifold("dl dt","dl dd ul li a","slow",1,"click");});*/
	$(".menu_dropdown dl dt").click(
		function(){
			if($(this).parent("dl").hasClass('selected')){
				$(this).parent("dl").removeClass('selected');
			}else{
				$(this).parent("dl").parent("div").children("dl").removeClass('selected');
				$(this).parent("dl").addClass("selected");
			}
		});
	/*左侧菜单选中效果及清除*/
	$(".menu_dropdown dl dt a").click(function(){if(curItem != null){curItem.children("b").remove();}});
	$(".Hui-nav li a").click(function(){if(curItem != null){curItem.children("b").remove();}});
	$(".menu_dropdown dl dd li a").click(function(){if(curItem != null){curItem.children("b").remove();};curItem=$(this).parent("li");$(this).parent("li").append("<b></b>");});
	/*文本框*/
